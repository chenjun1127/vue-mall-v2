    ...
    <div class="container">
      <div class="row">
        ...
        <template v-if="!noData">
          <table class="table table-hover table-bordered" v-for="item in orderList" :key="item._id">
            <thead>
              <tr>
                <th colspan="7">
            ...
            <tbody>
              <tr>
                <td colspan="2">
                  <div class="product" v-for="(e,index) in item.goodsList" :key="index">
                    <div>
                      <img class="cartImg" :src="require(`@/assets/products/${e.product.image}`)" />
                    </div>
                    <div>{{e.product.name}}</div>
                    <div>×{{e.sum}}</div>
                  </div>
                </td>
                <td width="100" :rowspan="item.goodsList.length">{{item.deliverName}}</td>
                ...
                <td width="100" :rowspan="item.goodsList.length">总额：￥{{formatPrice(item.totalMoney)}}</td>
              </tr>
            </tbody>
          </table>
        </template>
