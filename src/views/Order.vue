<template>
  <div>
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-4" v-for="item in resultList" :key="item._id">
        ...
        <div class="more" v-show="hasMore" @click="clickMore">
          MORE
          <span ref="span"></span>
        </div>
      </div>
          ...
          <table class="table" style="background: #f3fbfe">
            ...
            <tbody>
              <tr v-for="(item,index) in orderList" :key="index">
                <td>
                  <img class="cartImg" :src="require(`../assets/products/${item.image}`)" />
                </td>
                <td>{{item.name}}</td>
                <td>{{item.sum}}</td>
                <td>{{item.price}}</td>
      ...
      <div class="payTips">
        <p>
          <span>{{getTotal.totalSum}}</span>件商品，总商品金额：
          <span>￥{{formatPrice(getTotal.totalMoney)}}</span>
        </p>
        <p>
          返现：
          <span :style="{'color':cost.rebate === 0 ? '#333':'#d01a39' }">-￥{{formatPrice(cost.rebate)}}</span>
        </p>
        <p>
          运费：
          <span :style="{'color':cost.freight === 0 ? '#333':'#d01a39' }">￥{{formatPrice(cost.freight)}}</span>
        </p>
        <p>
          服务费：
          <span :style="{'color':cost.serviceCharge === 0 ? '#333':'#d01a39' }">￥{{formatPrice(cost.serviceCharge)}}</span>
        </p>
      </div>
      <div class="orderMsg">
        <p>
          应付总额：
          <em>￥{{formatPrice(getTotal.totalMoney+cost.freight+cost.serviceCharge-cost.rebate)}}</em>
        </p>
        <p>
          <i>
            <b>寄送至</b>
            {{getSelectedAddress.deliverAddress}}
          </i>
          <i>
            <b>收货人</b>
            {{getSelectedAddress.deliverName}}
          </i>
        </p>
      </div>
      <div class="payBtn">
        <router-link to="/to-cart" class="btn btn-success btn-lg">返回购物车</router-link>
          ...
          <div class="col-md-12">
            ...
            <form class="form-horizontal">
              <div class="form-group">
                ...
                <div class="col-md-10">
                  <v-distpicker :province="defaultPicker.province" :city="defaultPicker.city" :area="defaultPicker.area" @province="onChangeProvince" @city="onChangeCity" @area="onChangeArea" @selected="onSelected"></v-distpicker>
                </div>
              </div>
              <div class="form-group">
...
.active {
  ...
  &:before {
    ...
    display: block;
    color: #fff;
    background: url("../assets/svg/selected.svg");
