  ...
  <div class="cart" @mouseenter="mouseEnter" @mouseleave="mouseLeave">
    ...
    <div class="cartBox" :style="{'display':show ? 'block':'none'}">
      <div v-if="hasGoods">
        ...
        <div class="cartTotal">
          <p>
            共{{$store.state.cartList.length}}件商品，总计￥
            <span>{{formatPrice(totalPrice)}}</span>元
          </p>
          <p>
            <router-link to="/to-cart">去购物车</router-link>
          </p>
        </div>
      </div>
...
.cart {
  ...
  color: #e02f2f;
  em {
    background: url("../assets/images/sales-car.png") center no-repeat;
    width: 20px;
    height: 20px;
