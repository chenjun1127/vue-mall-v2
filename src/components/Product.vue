<template>
  <div class="row">
    <div class="col-md-3 card" v-for="item in list" :key="item._id">
      <div class="card-content">
        <div class="card-img" id="tt">
          <img class="card-img-top" v-lazy="require(`../assets/products/${item.image}`)" />
        </div>
        ...
        <div class="button-div">
          <button class="btn btn-danger pay-btn" @click="addCart(item._id,item.name,item.price,item.image)">加入购物车</button>
        </div>
      </div>
    </div>
  </div>
